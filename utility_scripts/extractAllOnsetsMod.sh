...
LOGFILE="logs/extractAllOnsetsMod.log"
mkdir -p logs
rm -f $LOGFILE
...
for audiofile in *.wav; do
	source ./utility_scripts/extractOnsetMod.sh $audiofile >> $LOGFILE
done

# In this modded version the timestamps have no delay added
